...
data "tls_certificate" "oidc" {
  url = aws_eks_cluster.NTI_eks.identity[0].oidc[0].issuer
}
...
data "aws_iam_policy_document" "aws_lbc_assume_role" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "${replace(aws_eks_cluster.NTI_eks.identity[0].oidc[0].issuer, "https://", "")}:sub"
      values   = ["system:serviceaccount:kube-system:aws-load-balancer-controller"]
    }

    condition {
      test     = "StringEquals"
      variable = "${replace(aws_eks_cluster.NTI_eks.identity[0].oidc[0].issuer, "https://", "")}:aud"
